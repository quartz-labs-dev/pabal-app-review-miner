...
- `백로그`의 기본 백로그 정렬은 `우선순위(MUST > SHOULD > COULD) → Status(Not Started > In Progress > Done) → Effort(High > Medium > Low) → 리뷰 수(내림차순)` 순서입니다.
...
- `리뷰 수`는 원문 인용 라인 수가 아니라, `reviewId` 기준으로 중복 제거한 고유 리뷰 수로 계산됩니다.
...
- `백로그` 탭에서 백로그를 페이지에서 직접 편집할 수 있습니다.
  - 백로그 항목 추가/삭제
  - 표 컬럼 순서는 `Priority → 백로그 항목 → Status → Effort → 리뷰 수 → 작업`입니다

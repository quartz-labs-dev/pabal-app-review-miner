...
- `리뷰`에서는 앱 필터를 제외한 다른 필터가 없을 때(`검색/태그/상태/100자 이상` 기본값) 페이지네이션 좌측에 북마크 UI가 표시됩니다. 북마크는 앱 필터별로 분리 저장되므로, 앱별 필터 화면마다 독립된 북마크를 가집니다. 북마크 저장은 아이콘 버튼으로 수행하며, 현재 북마크 페이지에서는 아이콘이 채워지고 다른 페이지에서는 비워진 상태로 표시됩니다. 북마크 요약은 `북마크 {페이지번호}` 형식으로 표시되며, `북마크로 이동` 버튼으로 저장된 북마크 페이지로 이동할 수 있습니다.

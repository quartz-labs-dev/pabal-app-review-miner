...
- `백로그`의 `근거 펼치기`는 필터 패널 내부에서 사용할 수 있습니다.
...
- `백로그`의 기본 백로그 정렬은 `우선순위(MUST > SHOULD > COULD) → Effort(High > Medium > Low) → 근거 수(내림차순)` 순서입니다.
...
- `백로그` 테이블에는 별도 `근거` 컬럼이 없으며, `근거 수` 옆 화살표 버튼으로 근거 행을 펼치고/접습니다.
...
- 상단 우측 컨트롤에 `노트` 버튼이 있으며, 우측 사이드바에서 백로그 셀렉터를 전환해 백로그 항목별 메모를 관리할 수 있습니다.
- 헤더 액션 버튼 `➕ 백로그 생성`은 `리뷰` 탭과 `백로그` 탭 모두에서 사용할 수 있습니다.
- 노트 패널의 백로그 셀렉터에서 현재 적용할 백로그 항목을 선택해 메모를 관리할 수 있습니다.
- 노트 사이드바에는 선택한 백로그 항목의 메타 정보(`Priority / Status / Effort / Evidence`)가 표시됩니다.
- 노트는 입력 중 자동 저장되며(`preview-state` 동일 경로), `저장` 버튼(또는 `Ctrl/Cmd + S`)으로 즉시 저장도 가능합니다.
...
- preview 모드에서는 카드 상태와 백로그 노트가 `data/{myAppId}/reports/preview-state.json`에 저장됩니다(둘 다 즉시 반영).
- `preview-state.json`은 리뷰 카드의 전체 상태(full-state)를 저장합니다(diff-only override 아님).
- `preview-state.json`은 v3 스키마(`reviews.tags`, `reviews.excluded`, `backlogNotes`)만 사용합니다. 기존 `favorite`/`notes`, `appNotes` 필드는 더 이상 사용하지 않습니다.
...
- `data/{myAppId}/reports/backlog.ko.json` (`status`를 포함한 통합 백로그 `items` 데이터, 근거는 `sourceToken::reviewId` scoped ID로 저장)
...
- `--keep-notes` (기본값 `true`): 리뷰 상태를 다시 초기화할 때 기존 백로그 노트 유지

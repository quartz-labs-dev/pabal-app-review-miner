...
- `리뷰`에서는 필터가 하나도 없을 때(`검색/태그/상태/100자 이상` 모두 기본값)만 페이지네이션 좌측에 북마크 UI가 표시됩니다. 북마크 저장은 아이콘 버튼으로 수행하며, 현재 북마크 페이지에서는 아이콘이 채워지고 다른 페이지에서는 비워진 상태로 표시됩니다. 북마크 요약은 `북마크 {페이지번호}` 형식으로 표시되며, `북마크로 이동` 버튼으로 저장된 북마크 페이지로 이동할 수 있습니다.

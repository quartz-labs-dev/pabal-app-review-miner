...
- `리뷰`에서는 앱 필터를 제외한 다른 필터가 없을 때(`검색/태그/상태/100자 이상` 기본값) 페이지네이션 좌측에 북마크 UI가 표시됩니다. 북마크는 앱 필터별로 분리 저장되므로, 앱별 필터 화면마다 독립된 북마크를 가집니다. 북마크 저장은 아이콘 버튼으로 수행하며, 현재 북마크 페이지에서는 아이콘이 채워지고 다른 페이지에서는 비워진 상태로 표시됩니다. 북마크 요약은 `북마크 {현재페이지}/{전체페이지}` 형식으로 표시되고, 마지막 페이지면 `북마크 {전체페이지}/{전체페이지} · 완료`로 표시됩니다. `북마크로 이동` 버튼으로 저장된 북마크 페이지로 이동할 수 있습니다.
- `리뷰`에서 북마크가 마지막 페이지인 경우 상단 필터 칩에 `검토 상태: 완료`가 표시됩니다.
- `리뷰`에서는 `100자 이상` 토글로 긴 리뷰만 빠르게 볼 수 있습니다.
- `필터 초기화` 버튼으로 검색/앱/상태/태그/길이 필터를 한 번에 해제할 수 있습니다.
...
- `리뷰` 탭의 각 리뷰 카드에는 단일 빠른 추가 셀렉터가 있으며, 백로그를 선택하면 즉시 연결 상태가 토글됩니다(미연결이면 `추가`, 이미 연결됨이면 `해제`).

...
- 좁은 화면에서 검색은 기본 `🔎` 버튼으로 축약되며, 버튼을 누르면 검색 입력창이 확장됩니다.
...
- 페이지네이션은 현재 검색/필터 결과를 기준으로 동작하며, 상단 요약에는 `전체 리뷰 수`만 표시됩니다.
- 제목 아래 요약/통계 블록도 탭에 따라 전환되어, 각 탭에 필요한 정보만 표시됩니다.
- `리뷰` 탭 상단 요약 블록에서는 `Raw 인용` 카드와 생성 시각/기준/분류 메타 목록을 표시하지 않습니다.
...
- 노트 탭 영역의 `전체 보기` 버튼으로 앱 목록 오버레이를 열고/닫아 전체 앱을 한 번에 선택할 수 있습니다.

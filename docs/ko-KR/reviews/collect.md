...
- `--global`: 스토어별 글로벌 마켓 목록(Play: 국가+언어, App Store: 국가)을 순회하며 리뷰 수집. 기본값은 활성화입니다. 글로벌 모드에서는 `--limit`이 마켓별로 적용됩니다.
...
- Google Play는 `googlePlay.packageName`을 사용합니다.
- App Store는 `appStore.appId`를 사용하며, `You Might Also Like` 우선 후 검색 폴백을 사용합니다.
- 자동 탐색은 먼저 후보 풀을 수집합니다(스토어당 `auto-top * 5`, 최대 `50`개).
- 이후 스토어별 `--auto-top` 개수만큼 성공 결과를 채웁니다.
- 자동 탐색 실수행(run)에서는 총 수집 리뷰가 `20개 이하`인 경쟁앱은 저장하지 않고, 같은 스토어의 다음 후보로 대체합니다.
